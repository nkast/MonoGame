...
namespace Microsoft.Xna.Framework
{
        ...
        , Google.VRToolkit.Cardboard.CardboardView.IRenderer
    {
        ...
        void processStatePausing()
        {
            ...
            // TNC: Fix crash when OpenAL is not initialized and OnPauseGameThread and OnResumeGameThread is null.
            var onPauseGameThreadHandler = OnPauseGameThread;
            if (onPauseGameThreadHandler != null)
                onPauseGameThreadHandler(this, EventArgs.Empty);
        ...
        void processStateResuming()
        {
            ...
            lock (_lockObject)
            {
                ...
                else if (glSurfaceAvailable) // finish state if surface created, may take a frame or two until the android UI thread callbacks fire
                {
                    ...
                    // TNC: Fix crash when OpenAL is not initialized and OnPauseGameThread and OnResumeGameThread is null.
                    var onResumeGameThreadHandler = OnResumeGameThread;
                    if (onResumeGameThreadHandler != null)
                        onResumeGameThreadHandler(this, EventArgs.Empty);
