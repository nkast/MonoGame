...
namespace Microsoft.Xna.Framework
{
    class MetroGamePlatform : GamePlatform
    {
        ...
        {
            if (MetroGameWindow.Instance.CoreWindow.CustomProperties.ContainsKey("RunInMainThread"))
                _runInMainThread = (bool)MetroGameWindow.Instance.CoreWindow.CustomProperties["RunInMainThread"];
            if (_runInMainThread)
            {
                if (!_enableRunLoop)
                {
